...
export function AlgorithmsPanel() {
  ...
  const tabs: { id: AlgorithmTab; label: string }[] = [
    { id: 'grover', label: 'GROV' },
    { id: 'dj', label: 'D-J' },
    { id: 'qft', label: 'QFT' },
    { id: 'teleport', label: 'TEL' },
    { id: 'qpe', label: 'QPE' },
    { id: 'vqe', label: 'VQE' },
    { id: 'qaoa', label: 'QAOA' },
    { id: 'qec', label: 'QEC' },
  ...
  return (
    <div className="h-full flex flex-col">
      ...
      <div className="border-b border-qborder">
        <div className="flex">
          {tabs.slice(0, 4).map((tab) => (
            <button
              key={tab.id}
              onClick={() => setActiveTab(tab.id)}
              className={`flex-1 px-1 py-1.5 text-[10px] font-bold uppercase ${
                activeTab === tab.id
            ...
            {[
              ...
              <button key={opt.value}
                ...
                className={`flex-1 py-1 border font-bold ${
                  oracleType === opt.value ? 'border-accent bg-accent text-gray-800' : 'border-qborder hover:border-accent'
                }`}>
                {opt.label}
              </button>
            ))}
      ...
      {result && (
        <div className="space-y-2 pt-2 border-t border-gray-200">
          ...
          <div className="flex justify-between">
            ...
            <span className={result.correct ? 'text-gray-800' : 'text-gray-400'}>
...
function QFTPanel() {
  ...
  return (
    <div className="space-y-3 text-xs">
      ...
      <div className="text-gray-600 text-[10px]">Transform to frequency domain</div>
          ...
          <div className="flex gap-1 mt-1">
            {Array.from({ length: nQubits }, (_, i) => (
              <button key={i}
                ...
                className={`flex-1 py-1 border font-bold ${
                  inputState[i] === 1 ? 'border-accent bg-accent text-gray-800' : 'border-qborder'
                }`}>
                {inputState[i]}
              </button>
            ))}
          ...
          {Object.entries(result.counts)
            ...
            .map(([state, count]) => (
              <div key={state} className="flex items-center gap-2">
                ...
                <div className="flex-1 h-3 border border-gray-200 bg-black">
                  <div className="h-full bg-white" style={{ width: `${(count / result.shots) * 100}%` }} />
                </div>
                <span className="w-10 text-right text-gray-400">{((count / result.shots) * 100).toFixed(0)}%</span>
              </div>
            ))}
        </div>
      )}
...
function TeleportationPanel() {
  ...
  return (
    <div className="space-y-3 text-xs">
      ...
      <div className="text-gray-600 text-[10px]">Transfer state using entanglement</div>
          ...
          <div>
            ...
            <div className="grid grid-cols-3 gap-1 text-center">
              {(['x', 'y', 'z'] as const).map((axis) => (
                <div key={axis} className="border border-gray-200 p-1">
                  <div className="text-gray-600">{axis.toUpperCase()}</div>
                  <div>{result.input_bloch[axis].toFixed(2)}</div>
                </div>
              ))}
            </div>
          </div>
          <div>
            ...
            <div className="grid grid-cols-3 gap-1 text-center">
              {(['x', 'y', 'z'] as const).map((axis) => (
                <div key={axis} className="border border-gray-200 p-1">
                  <div className="text-gray-600">{axis.toUpperCase()}</div>
                  <div>{result.output_bloch[axis].toFixed(2)}</div>
                </div>
              ))}
            </div>
          </div>
